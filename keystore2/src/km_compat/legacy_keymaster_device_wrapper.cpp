...
Return<void> LegacyKeymasterDeviceWrapper::getHardwareFeatures(getHardwareFeatures_cb _hidl_cb) {
    _hidl_cb(true, true, true, true, true, "Google", "Google Android Security");
    return Void();
}
...
sp<IKeymasterDevice> makeSoftwareKeymasterDevice() {
    ...
    dev = (new SoftKeymasterDevice(keymaster::KmVersion::KEYMASTER_3))->keymaster2_device();
